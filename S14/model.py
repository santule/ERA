...
class PositionEncoding(nn.Module):
  ...
  def __init__(self,d_model:int, seq_len: int, dropout: float) -> None:
    ...
    pe = torch.zeros(seq_len, d_model) # 350, 512

    position = torch.arange(0, seq_len, dtype =torch.float).unsqueeze(1) # 350,1
    div_term = torch.exp(torch.arange(0, d_model,2).float() * (-math.log(10000) / d_model)) # 256

    pe[:,0::2] = torch.sin(position * div_term) # 350,256
    pe[:,1::2] = torch.cos(position * div_term) # 350,256

    pe = pe.unsqueeze(0) #1, 350,512
...
class Transformer(nn.Module):
  ...
  def decode(self,encoder_output: torch.Tensor, src_mask: torch.Tensor, tgt, tgt_mask: torch.Tensor):
